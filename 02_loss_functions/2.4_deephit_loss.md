...
| ✅ **Competing risks are native**: one head per cause, cause-specific CIFs ([`1.4`](../01_background/1.4_multi_events_and_competing_risks.md)). | ❌ **Two hyperparameters** ($\alpha$, $\sigma$) to tune, plus the bin grid, so more knobs than Cox. |
