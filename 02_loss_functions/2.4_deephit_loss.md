...
DeepHit (Lee et al., 2018) is a deep-learning survival model that makes **no assumptions about the shape of risk**: not the proportional hazards of Cox ([`2.1`](2.1_cox_ph_loss.md)), and not a parametric distribution like the NLL models ([`2.2`](2.2_negative_log_likelihood_loss.md)). Instead it lets a neural network output the survival distribution *directly* as a discrete probability table, and it handles **competing risks** ([`1.4`](../01_background/1.4_multi_events_and_competing_risks.md)) natively. Its loss combines a likelihood term (for calibration) with a ranking term (for discrimination).
...
\qquad
\underbrace{S(t \mid x)}_{\text{overall survival}}
...
![Left: the network emits one probability per (cause, time-bin) cell — the joint mass $p_k(t\mid x)$. Right: everything else is read off by summing those cells — each cause's CIF is a running sum of its cells, and overall survival $S(t)$ is 1 minus every cell so far. No second model needed.](../assets/figures/2.4_deephit_pmf.svg){fig-alt="Left: grouped bar chart of probability mass for two causes across time bins. Right: cumulative incidence step curves for the two causes plus a descending overall survival curve."}

## From data to loss

DeepHit is trained by minimizing a **weighted sum of two losses** that target the two things we want from a survival model:

$$
\underbrace{L}_{\text{total}} \;=\; \underbrace{L_{\text{likelihood}}}_{\text{calibration: right probabilities}} \;+\; \alpha \cdot \underbrace{L_{\text{rank}}}_{\text{discrimination: right ordering}}
$$

The hyperparameter $\alpha \ge 0$ sets how much ranking matters relative to calibration. We build up each term below.

### Term 1 — the likelihood loss (calibration)
...
The likelihood term alone makes probabilities *calibrated* but does not directly reward putting patients in the **right order** — which is what the [C-index](../03_metrics/3.1_c_index.md) measures. The ranking term adds that pressure with a smooth, differentiable penalty over **comparable pairs**.
...
$$
L \;=\; L_{\text{likelihood}} \;+\; \alpha \cdot L_{\text{rank}}
$$

Calibration ($L_{\text{likelihood}}$) and discrimination ($\alpha L_{\text{rank}}$) can pull in different directions; $\alpha$ is the knob that balances them.
...
| ✅ **Optimizes calibration *and* ranking** at once. | ❌ **Data-hungry** — predicting a full PMF needs more events than fitting a handful of coefficients. |
...
- **It outputs full probabilities** — per-cause CIFs $F_k(t)$ and overall $S(t)$ — not just a ranking. Evaluate both: the [C-index](../03_metrics/3.1_c_index.md) for discrimination *and* the Brier score / calibration for probability quality.
- **`alpha` and `sigma` are the two knobs that move the needle.** `alpha` trades calibration against discrimination; `sigma` sets how sharply mis-ordered pairs are punished. The defaults rarely win — tune both. (More bins = finer time resolution but more parameters and sparser cells.)
...
model = DeepHitSingle(net, optimizer,
                      alpha=0.2, sigma=0.1,        # alpha weights likelihood vs ranking; sigma is the ranking scale
